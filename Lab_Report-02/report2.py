def limited_depth_search(grid, pos, goal, current_depth, max_depth, explored, route):
    ...
    for dr, dc in directions:
        ...
        if (
            0 <= new_r < total_rows and 0 <= new_c < total_cols
            and grid[new_r][new_c] == 0 and next_pos not in explored
        ):
            found, found_route = limited_depth_search(
                grid, next_pos, goal, current_depth + 1, max_depth, explored, route
            )
            if found:
