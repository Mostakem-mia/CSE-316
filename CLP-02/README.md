<h1>CLP-02<h1>

<h2>Code Description and Output</h2>

<h2 id="list-duplicates">01. List: Given a list of numbers, remove duplicates and sort in ascending order.</h2>
<p>This program takes a list of numbers, removes any duplicate values, and sorts them in ascending order. 
It ensures that each number appears only once and organizes them in increasing order.</p>
<p><strong>Example Input:</strong></p>
<pre>list: [1,4,5,2,5,2,1,7,6,5,8,8,9]</pre>
<p><strong>Example Output:</strong></p>
 <pre><img src="images/1.png" alt="list" width="500"></pre>

<h2 id="set-common-elements">02. Set: Find the common elements between two lists using sets.</h2>
<p>This program identifies the common numbers between two lists using Python sets. 
It efficiently finds shared values between two datasets.</p>
<p><strong>Example Input:</strong></p>
<pre>list1: [1, 2, 3, 4, 5]
list2: [3, 4, 5, 6, 7]</pre>
<p><strong>Example Output:</strong></p>
 <pre><img src="images/2.png" alt="list" width="500"></pre>

<h2 id="tuple-student-grades">03. Tuple: Create a tuple of student records (name, age, grade) and sort by grade.</h2>
<p>This program stores student records as tuples and sorts them based on grades in ascending order.</p>
<p><strong>Example Input:</strong></p>
<pre>students: [("Mostakem", 20, 85), ("Atik", 22, 78), ("Shakib", 21, 90)]</pre>
<p><strong>Example Output:</strong></p>
 <pre><img src="images/3.png" alt="list" width="500"></pre>

<h2 id="dictionary-word-count">04. Dictionary: Count word occurrences in a given text and store them in a dictionary.</h2>
<p>This program counts the occurrences of each word in a given text and stores the counts in a dictionary.</p>
<p><strong>Example Input:</strong></p>
<pre>text: "hello this is mostakem mia giving you hello"</pre>
 <pre><img src="images/4.png" alt="list" width="500"></pre>

<h2 id="numpy-matrix-sum">05. NumPy#1: Generate a 5x5 matrix of random integers and compute row-wise sums.</h2>
<p>This program generates a 5x5 matrix with random numbers and computes the sum of each row separately.</p>
<p><strong>Example Output:</strong></p>
 <pre><img src="images/5.png" alt="list" width="500"></pre>

<h2 id="numpy-normalize">06. NumPy#2: Create an array of 100 random values and normalize them between 0 and 1.</h2>
<p>This program generates 100 random values and normalizes them between 0 and 1 using min-max normalization.</p>
<p><strong>Example Output:</strong></p>
 <pre><img src="images/6.png" alt="list" width="500"></pre>

<h2 id="pandas-total-revenue">07. Pandas#1: Load a CSV file of sales data and compute total revenue per product.</h2>
<p>This program reads sales data from a CSV file, groups the revenue by product type, and computes the total revenue.</p>
<p><strong>Example CSV Input:</strong></p>
<pre>Product,Revenue
A,100
B,200
A,150
C,300
B,250
C,400
A,200
B,150
C,350
A,300</pre>
<p><strong>Example Output:</strong></p>
 <pre><img src="images/7.png" alt="list" width="500"></pre>

<h2 id="pandas-missing-values">08. Pandas#2: Fill missing values in a dataset with column-wise means.</h2>
<p>This program fills missing values (None) in a dataset by replacing them with the mean of their respective columns.</p>
<p><strong>Example Input:</strong></p>
<pre> A     B
10    50
20   None
None  15
40    10</pre>
<p><strong>Example Output:</strong></p>
 <pre><img src="images/8.png" alt="list" width="500"></pre>

<h2 id="matplotlib-line-graph">09. Matplotlib#1: Plot a line graph showing temperature variations over a week.</h2>
<p>This program plots a line graph that visualizes temperature changes throughout the week, showing fluctuations and trends over time.</p>
<p><strong>Example Input:</strong></p>
<pre>days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
temps = [31, 34, 33, 29, 28, 27, 26]</pre>
<p><strong>Example Output:</strong></p>

 <pre><img src="images/9.png" alt="list" width="500"></pre>

<h2 id="matplotlib-bar-chart">10. Matplotlib#2: Create a bar chart comparing sales revenue across different regions.</h2>
<p>This program creates a bar chart comparing sales revenue in different regions.</p>
<p><strong>Example Input:</strong></p>
<pre>regions = ["North", "South", "East", "West"]
revenues = [50000, 60000, 45000, 70000]</pre>
<p><strong>Example Output:</strong></p>
 <pre><img src="images/10.png" alt="list" width="500"></pre>

